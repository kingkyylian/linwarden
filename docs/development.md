...
Pushing a tag like `v0.9.0` runs tests, compile, Ruff, and mypy, builds source and wheel artifacts, creates `dist/SHA256SUMS`, optionally signs that checksum manifest when GPG secrets are configured, then creates a GitHub Release with generated notes.
